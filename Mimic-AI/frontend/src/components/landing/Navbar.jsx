...
export default function Navbar() {
  ...
  return <header className={`nav-wrap ${scrolled ? 'scrolled' : ''}`}>
    <nav className="nav container">
      <a className="brand" href="#top"><Logo compact /></a>
      <div className="nav-links">{links.map(link => <a href={`#${link.toLowerCase().replaceAll(' ', '-')}`} key={link}>{link}</a>)}</div>
      <a href="/login" className="nav-cta">Create your profile <span>↗</span></a>
      <button className="menu-button" onClick={() => setOpen(!open)} aria-label="Toggle menu">{open ? <X /> : <Menu />}</button>
    </nav>
    {open && <div className="mobile-menu">{links.map(link => <a onClick={() => setOpen(false)} href={`#${link.toLowerCase().replaceAll(' ', '-')}`} key={link}>{link}</a>)}<a href="/login">Create your profile</a></div>}
  </header>
}
